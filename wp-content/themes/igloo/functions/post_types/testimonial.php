...
function ci_create_cpt_testimonials() {
	...
	$args = array(
		...
		'hierarchical'    => false,
		'has_archive'     => _x( 'testimonials', 'post type archive slug', 'ci_theme' ),
		'rewrite'         => array( 'slug' => _x( 'testimonials', 'post type slug', 'ci_theme' ) ),
		'menu_position'   => 5,
...
{
	?><label for="title"><?php _e('Use the main title, to denote who made the testimonial.', 'ci_theme'); ?></label><?php
}
endif;
...
if( !function_exists('testimonial_edit_columns') ):
function testimonial_edit_columns($columns){  
	
	$new_columns = array(
		"cb" => $columns['cb'],
		"title" => __('Testimonial by:', 'ci_theme')
	);
