		...
		<div class="redi-admin-right" >
			...
			<div class="postbox">
				...
				<div class="inside">
					...
					<p><?php _e('News', 'redi-restaurant-reservation') ?>: <a target="_blank" href="https://www.facebook.com/ReDiReservation">News</a></p>
					<p><?php _e('User guideline', 'redi-restaurant-reservation') ?>: <a target="_blank" <a href="http://plugin.reservationdiary.eu/wp-content/uploads/2017/04/ReDi-Restaurant-Reservation-Plugin-Userguide.pdf">Download</a></p>
					<p><?php _e('Support', 'redi-restaurant-reservation') ?>: <a target="_blank" href="https://goo.gl/rAZv1E">Create a support ticket</a>
...
<div class="tab_wrap <?php if(!isset($_GET['sm']) || (isset($_GET['sm']) && $_GET['sm']=='free')): ?>redi-admin-left<?php endif ?>">
	...
	<form name="redi-restaurant" method="post">
        ...
        <th scope="row">
			...
			<td style="vertical-align: top;">
				<input type="checkbox" name="ManualReservation" id="ManualReservation" value="1" <?php if (isset($manualReservation) && $manualReservation) echo 'checked="checked"' ?>>
			</td>
			<td style="width:75%">
				<p class="description">
            ...
            <tr>
                ...
                <td style="vertical-align: top;">
                    <select name="AlternativeTimeStep">
                        ...
                        <option value="90" <?php if ($alternativeTimeStep == 90):?>selected="selected" <?php endif;?>><?php printf(__('%d min', 'redi-restaurant-reservation'), 90);?></option>
						<option value="120" <?php if ($alternativeTimeStep == 120):?>selected="selected" <?php endif;?>><?php printf(__('%d min', 'redi-restaurant-reservation'), 120);?></option>
                    </select>
                </td>
                <td style="width:75%">
                    <p class="description">
            ...
            <tr>
                ...
                <td style="width:75%">
                    <p class="description">
                        ...
                        <b><?php _e('From WordPress email account', 'redi-restaurant-reservation') ?></b> - <?php _e('The confirmation email will be sent out from your email set in WordPress.', 'redi-restaurant-reservation') ?><br/>
		...
		<table class="form-table" style="width: 90%; text-align: center;">
			...
			<tr>
				<td>
                    <input type="hidden" id="<?php echo $field_id;?>" name="<?php echo $field_id;?>" value="<?php  echo $custom_field->Id ?>" />
					<?php $field_name=('field_'.$i.'_name'); ?>
					<input type="text" id="<?php echo $field_name; ?>" name="<?php echo $field_name; ?>" value="<?php echo $custom_field->Name; ?>"/>
				</td>
				<td style="vertical-align: top;">
					<?php $field_type=('field_'.$i.'_type'); ?>
					<select onchange="set_name('<?php echo $field_name; ?>',this.options[this.selectedIndex].value);" class="field_type" name="<?php echo $field_type; ?>" id="<?php echo $field_type; ?>">
						<option value="text" <?php if ($custom_field->Type === 'text'):?>selected="selected"<?php endif?>><?php _e('Text field', 'redi-restaurant-reservation'); ?></option>
						<option value="checkbox" <?php if ($custom_field->Type === 'checkbox'):?>selected="selected"<?php endif?>><?php _e('Check box', 'redi-restaurant-reservation'); ?></option>
						<option value="reminder" <?php if ($custom_field->Type === 'reminder'):?>selected="selected"<?php endif?>><?php _e('Reminder', 'redi-restaurant-reservation'); ?></option>
						<option value="newsletter" <?php if ($custom_field->Type === 'newsletter'):?>selected="selected"<?php endif?>><?php _e('Newsletter', 'redi-restaurant-reservation'); ?></option>
						<option value="allowsms" <?php if ($custom_field->Type === 'allowsms'):?>selected="selected"<?php endif?>><?php _e('Allow SMS', 'redi-restaurant-reservation'); ?></option>
					</select>
				</td>
				<td>
					<?php $field_required=('field_'.$i.'_required'); ?>
					<input type="checkbox" id="<?php echo $field_required; ?>" name="<?php echo $field_required; ?>" <?php if ($custom_field->Required): ?>checked="checked"<?php endif?>>
				</td>
				<td>
					<?php $field_message=('field_'.$i.'_message'); ?>
					<input type="text" id="<?php echo $field_message;?>" name="<?php echo $field_message;?>" value="<?php echo $custom_field->Message ?>" style="width:250px;">
				</td>
                <td>
                    <a onclick="jQuery('#<?php echo $field_name; ?>').val(''); jQuery('#<?php echo $field_message; ?>').val(''); jQuery('#<?php echo $field_type; ?>').val('text'); jQuery('#<?php echo $field_required; ?>').attr('checked', false);">clear</a>
                </td>
			</tr>
        ...
        <?php elseif((isset($_GET['sm']) && $_GET['sm']=='basic')):?>
            <iframe src="//wp.reservationdiary.eu/<?php echo str_replace( '_', '-', get_locale() )?>/<?php echo $this->ApiKey; ?>/Home" width="100%;" style="min-height: 1700px;"></iframe>
