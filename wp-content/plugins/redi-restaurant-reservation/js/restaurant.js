...
jQuery(function () {
    ...
    var date = new Date(jQuery('#redi-restaurant-startDateISO').val())
	date.setMinutes(date.getTimezoneOffset());
	jQuery('#redi-restaurant-startDate').datepicker("setDate", date);
    ...
    jQuery(document).on('click', '#redi-restaurant-step3', function () {
        ...
        if (!validatePhone(jQuery('#UserPhone').val()))
        {
        ...
        {
            if (jQuery(custom_fields[index]).attr('type') === 'checkbox' && jQuery(custom_fields[index]).attr('checked') === 'checked') 
            {
