...
if ( ! class_exists( 'ReDi' ) ) {
	class Redi {
		...
		public function updateCustomField( $lang, $placeID, $customFieldID, $params ) {
			return $this->request( REDI_RESTAURANT_API . CUSTOMFIELDS . $lang . '/' . $this->ApiKey . '/place/' . $placeID . '/customfield/' . $customFieldID, PUT,
				json_encode( self::unescape_array( $params ) ) );
		}

		public function saveCustomField( $lang, $placeID, $params ) {
			return $this->request( REDI_RESTAURANT_API . CUSTOMFIELDS . $lang . '/' . $this->ApiKey . '/place/' . $placeID, POST,
				json_encode( self::unescape_array( $params ) ) );
		}
